...
class Node():
    class Type (Enum):
        ...
        FOR_LOOP    = auto() # A deterministic for loop. Gets unwrapped at compilation / runtime at the same scope.
...
def constructAST (tokens: list[Token]) -> Node:
    ...
    def processForLoop (tokens: list[Token], for_kw_index: int) -> tuple[Node, int]:
        ...
        return (Node(Node.Type.FOR_LOOP, for_kw=tokens[for_kw_index], var=var, begin=begin, end=end, step=step, body=body), close_curly_index +1)
...
def constructProgram (ast: Node) -> Instruction:
        ...
        def addFunc (self, func_def: Node) -> None:
            ...
            def validateScopeFuncCalls (content: list[Node], parent_scope: Scope, starter: Token) -> None:
                ...
                for node in content:
                    if node.type == Node.Type.VAR_ASSIGN:
                    ...
                    elif node.type == Node.Type.FOR_LOOP:
                        ...
                        # still need to check body, but not in a new scope
    ...
    def evaluateScope (content: list[Node], scope: Scope | tuple[Token , Scope | None]) -> Number | Instruction:
        '''Processes / evaluates a scope and returns 
        ...
        def processContent (content: list[Node], scope: Scope) -> None: # FIXME, yak a chief. W return n7tha f my ass? # How about we simply and literally unwrap the for loops and append the nodes to the content and let it compute
            '''Simply processes the given content (list of instruction Nodes)
            in the given scope. Returns nothing'''
            ...
            assert scope is not None, f"Scope is None"
            ...
            for node in content:
                if node.type == Node.Type.VAR_ASSIGN:
                    ...
                    parameters = [
                        node.components['begin'],
                        node.components['end'],
                        node.components['step'],
                    ]
                    for i, parameter in enumerate(parameters):
                        if type(parameter) == Instruction:
                            parameters[i] = parameter.justEvaluate()
                    var = node.components['var']
                    begin, end, step = parameters
                    body = node.components['body']
                    if step == 0:
                        invalidCode(f"For loops can't have a null step (infinite loop). This for loop step was evaluated and it was zero", node.components['for_kw'])
                    elif step > 0:
                        while begin <= end:
                            scope.setVarState(var, False, begin)
                            processContent(body, scope)
                            begin += step
                    else:
                        while end >= begin:
                            scope.setVarState(var, False, end)
                            processContent(body, scope)
                            end += step
